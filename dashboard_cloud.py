...
# NEW – équivalences pizza / pâté lorrain
st.markdown("**🍕🥧 Équivalences gourmandes (approx.)**")

# on regroupe les sports vélo et CAP
CYCLE_SPORTS = [
    "Ride",
    "VirtualRide",
    "GravelRide",
...
calories_velo = float(
    by_sport.loc[by_sport["sport"].isin(CYCLE_SPORTS), "total_kcal"].sum()
)
calories_cap = float(
    by_sport.loc[by_sport["sport"].isin(RUN_SPORTS), "total_kcal"].sum()
)

pizza_parts = calories_velo / CAL_PER_PIZZA_SLICE if calories_velo > 0 else 0.0
pate_parts = calories_cap / CAL_PER_PATE_LORRAIN if calories_cap > 0 else 0.0

col_pizza, col_pate = st.columns(2)
with col_pizza:
    st.metric(
        "Vélo → parts de pizza",
        f"{pizza_parts:.1f} parts",
        help=f"Basé sur ~{CAL_PER_PIZZA_SLICE:.0f} kcal par part de pizza.",
    )
with col_pate:
    st.metric(
        "Course à pied → parts de pâté lorrain",
        f"{pate_parts:.1f} parts",
        help=f"Basé sur ~{CAL_PER_PATE_LORRAIN:.0f} kcal par part de pâté lorrain.",
    )
...
st.dataframe(
    df_display[
        ["date", "sport", "name", "distance (km)", "temps (min)", "calories (kcal)"]
    ],
    use_container_width=True,
    height=300,
)
